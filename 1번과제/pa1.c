...
static int run_command(int nr_tokens, char *tokens[])
{
    ...
    int start = 0;
    int end = pipe_pt; /* pipe 있는 곳 */
    int status;
    
    command_div(tokens, start, end , argv); /*left process*/
    command_div(tokens, pipe_pt+1, nr_tokens , args); /*right process*/
    ...
    if(pipe_check == 1) /*pipe exist*/
    {
        ...
        else if(p1 == 0) /*fork 성공*/
        {
            dup2(fd[1],STDOUT_FILENO);
            execvp(argv[0],argv);
        }
        
        p2 = fork(); /*read, right command*/
...
static void append_history(char * const command)
{
    ...
    return_value->string = (char *)malloc(strlen(command)+1); /* \0 포함🤦🏻‍♀️ */
